...
/**
 * Query options for `GET /api/v2/torrents/info`.
 */
export interface GetTorrentListOptions {
  /**
   * Filter by one or more torrent hashes.
   */
  ...
  /**
   * Sort key for torrent list (e.g. `name`, `progress`, `dlspeed`).
   */
  sort?: string;
  ...
  /**
   * Whether to include trackers in each torrent item.
   */
...
export interface AddNewTorrentOptions {
   ...
   * Multiple tags are comma-separated in request payload.
   */
  tags: string | string[];

  /**
   * Skip hash checking.
   * Omit to use server default (`false`).
   */
  skip_checking?: boolean;

  /**
   * Add torrents in the paused/stopped state.
   * Omit to use server default (`false`).
   */
  paused?: boolean;

  /**
   * Add torrents in the stopped state (v5+).
   * Omit to use server default (`false`).
   */
  stopped?: boolean;
...
export interface AddNewMagnetOptions {
  savepath: string;
   ...
   * Omit to keep server behavior (unset/default).
   */
  root_folder?: boolean;
...
export type Peers = Record<string, TorrentPeer>;
